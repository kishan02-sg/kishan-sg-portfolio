...
export default function VideoIntro() {
  ...
  const startedRef  = useRef(false)   // becomes true once the user clicks "Start"
  ...
  useEffect(() => {
    function onLoaderDismissed() {
      ...
      // Play INSIDE the click gesture so unmuted playback is allowed by the browser
      v.play().catch(() => {})
      dismissHint()
    }
  ...
  // Pause the video (and its audio) when scrolled away from the intro; resume only at the top.
  // The intro is position:sticky so it never "leaves" the viewport — watch scrollTop instead.
  useEffect(() => {
    const v = videoRef.current
    const main = document.querySelector('main')
    if (!v || !main) return
    function onScroll() {
      if (typeof v.play !== 'function') return
      const scrolledAway = main.scrollTop > window.innerHeight * 0.5
      if (scrolledAway) {
        if (!v.paused) v.pause()
      } else if (startedRef.current && v.paused) {
        v.play().catch(() => {})   // back at the top → resume (only if intro was started)
      }
    }
  ...
  return (
    <section ref={sectionRef} className={styles.section}>
      ...
      <video
        src="/assets/kishan-intro-v2.mp4"
